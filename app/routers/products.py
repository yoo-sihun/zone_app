...
def create_product(data: ProductIn, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    product = Product(user_id=user.id, name=data.name, ingredients=data.ingredients)
    db.add(product)
    db.commit()
    db.refresh(product)
    return product
