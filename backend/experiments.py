...
def get_active_experiment(db: Session) -> Experiment | None:
    return db.query(Experiment).filter(Experiment.status == "active").first()
...
def locked_ingredient(db: Session, check_date: Date) -> str | None:
...
def compute_result(db: Session, experiment: Experiment) -> dict:
    ...
    before_count = (
        db.query(TroubleDot)
        .filter(TroubleDot.date >= before_start, TroubleDot.date <= before_end)
        .count()
    )
    during_count = (
        db.query(TroubleDot)
        .filter(TroubleDot.date >= start, TroubleDot.date <= during_end)
        .count()
    )
