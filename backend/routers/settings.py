...
def _get_or_create(db: Session) -> AppSetting:
    """싱글턴 설정 행 — 없으면 기본값(AI 켜짐)으로 하나 만듦. 헤더(X-Profile-Id) 불필요 —
    프로필과 무관한 앱 전역 설정이라 /api/config, /api/profiles와 같은 성격."""
    row = db.query(AppSetting).first()
    if not row:
        row = AppSetting(ai_enabled=True)
        db.add(row)
        db.commit()
        db.refresh(row)
    return row
...
def update_ai_setting(data: AISettingIn, db: Session = Depends(get_db)):
    ...
    db.commit()
    set_enabled(data.ai_enabled)  # 이번 프로세스에 즉시 반영(재배포/재시작 불필요)
    return {"ai_enabled": row.ai_enabled}
