...
이건 백엔드(API 전용)만 띄우는 것 — `http://127.0.0.1:8000/docs`에서 API 문서 확인 가능. 화면(프론트)까지 보려면 별도 터미널에서 `cd frontend && npm install && npm run dev`로 Next.js 개발 서버를 같이 띄워야 함(기본 3000번 포트).
